...
var (
	...
	alphaPattern    = regexp.MustCompile(`^[A-Za-zÁÉÍÓÚáéíóúÑñÜü]+$`)
	alphanumPattern = regexp.MustCompile(`^[A-Za-z0-9ÁÉÍÓÚáéíóúÑñÜü]+$`)
...
func required(f Field) error {
	...
	if f.Value == nil || reflect.ValueOf(f.Value).IsZero() {
		return fmt.Errorf("el campo [%s] es requerido", f.Path)
	}
...
func min(f Field) error {
	...
	if s, ok := f.Value.(string); ok {
		whole, err := strconv.Atoi(f.Param)
		if err != nil {
			return fmt.Errorf("parámetro inválido para min en [%s]", f.Path)
		}
		if utf8.RuneCountInString(s) < whole {
			return fmt.Errorf("el campo [%s] debe tener mínimo [%d] caracteres", f.Path, whole)
		}
		return nil
	}

	num, ok := asRat(f.Value)
	if !ok {
		return fmt.Errorf("min solo soporta string o número en [%s]", f.Path)
	}

	limit, ok := new(big.Rat).SetString(f.Param)
	if !ok {
		return fmt.Errorf("parámetro inválido para min en [%s]", f.Path)
	}
	if num.Cmp(limit) < 0 {
...
func max(f Field) error {
	...
	if s, ok := f.Value.(string); ok {
		...
		if utf8.RuneCountInString(s) > whole {
			return fmt.Errorf("el campo [%s] debe tener máximo [%d] caracteres", f.Path, whole)
		}
		return nil
	}
...
func email(f Field) error {
	...
	if err != nil || parsed.Address != s || !emailPattern.MatchString(s) {
...
func timeValue(f Field) error {
	...
	case time.Time:
		return nil
	default:
		return fmt.Errorf("time solo soporta string o time.Time en [%s]", f.Path)
	}
...
func greaterThan(f Field) error {
	return compareNumber(f, func(result int) bool { return result > 0 }, "mayor que")
}

func greaterThanOrEqual(f Field) error {
	return compareNumber(f, func(result int) bool { return result >= 0 }, "mayor o igual que")
}

func lessThan(f Field) error {
	return compareNumber(f, func(result int) bool { return result < 0 }, "menor que")
}

func lessThanOrEqual(f Field) error {
	return compareNumber(f, func(result int) bool { return result <= 0 }, "menor o igual que")
}

func compareNumber(f Field, cmp func(int) bool, label string) error {
	value, ok := asRat(f.Value)
	if !ok {
		return fmt.Errorf("[%s] solo soporta números", f.Tag)
	}

	param, ok := new(big.Rat).SetString(f.Param)
	if !ok {
		return fmt.Errorf("parámetro inválido para [%s] en [%s]", f.Tag, f.Path)
	}

	if !cmp(value.Cmp(param)) {
